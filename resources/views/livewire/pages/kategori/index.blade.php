<div>
    <x-slot name="breadcrumb">
        <livewire:nawasara-ui.shared-components.breadcrumb
            :items="[['label' => 'Hibah & Bansos', 'url' => '#'], ['label' => 'Master Kategori']]" />
    </x-slot>

    <x-nawasara-ui::page.container>
        <x-nawasara-ui::page-header
            title="Master Kategori"
            description="Kategori hibah/bansos yang dapat dipilih saat entry pengajuan."
            :count="$this->rows->count().' kategori'">
                    ...
                    <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-700/40">
                        ...
                        <td class="px-4 py-2.5 text-right">
                            <div class="inline-flex items-center gap-1">
                                <x-nawasara-ui::icon-button icon="pencil" tooltip="Edit"
                                    wire:click="edit({{ $kat->id }})" />
                                <x-nawasara-ui::icon-button
                                    :icon="$kat->aktif ? 'toggle-right' : 'toggle-left'"
                                    :tooltip="$kat->aktif ? 'Nonaktifkan' : 'Aktifkan'"
